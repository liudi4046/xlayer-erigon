...
wait_for_l1_batch() {
    ...
    while true; do
        ...
        if [ "$batch_type" = "virtual" ]; then
            ...
            echo "Invalid batch type. Use 'virtual'."
            return 1
        fi
...
L2_VIRTUALIZED_BATCH=$(wait_for_l1_batch 600 "virtual")
if [[ $? -ne 0 || -z "$L2_VIRTUALIZED_BATCH" ]]; then
    echo "Failed to wait for batch virtualization or get the virtualized batch number"
    exit 1
fi
...
echo "Using integration tool to unwind to batch $L2_VIRTUALIZED_BATCH"
kurtosis service exec cdk-v1 cdk-erigon-sequencer-001 "integration state_stages_zkevm --config=/etc/cdk-erigon/config.yaml --unwind-batch-no=$L2_VIRTUALIZED_BATCH --chain dynamic-kurtosis --datadir /home/erigon/data/dynamic-kurtosis-sequencer $INTEGRATION_TOOL_EXTRA_FLAGS"
